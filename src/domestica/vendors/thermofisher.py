...
class ThermoFisherSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="DOMESTICA_THERMOFISHER_", env_file=".env", extra="ignore"
    )
...
class ThermoFisherEvaluator(ComplexityEvaluator):
    ...
    def _get_token(self, force_refresh: bool = False) -> str:
        ...
        try:
            ...
            logger.debug(
                "ThermoFisher OAuth2 token successfully established. Valid for %s seconds.",
                body.get("expires_in", 3600),
            )
            return self._token
        except Exception:
            logger.exception(
                "Critical exception thrown while requesting authentication token from ThermoFisher endpoint."
            )
            raise
    ...
    def evaluate(self, sequence: str) -> Tuple[bool, Optional[float]]:
        ...
        while True:
            logger.debug(
                "Dispatching validation request to ThermoFisher API. Sequence length: %d bp",
            ...
            try:
                ...
                if response.status_code == 401:
                    logger.warning(
                        "ThermoFisher API token invalidated (401). Forcing token refresh."
                    )
                    self._get_token(force_refresh=True)
                    continue
                if response.status_code == 429:
                    logger.warning(
                        "ThermoFisher rate limit reached (429). Dormant cooling backoff for 10 seconds."
                    )
                    time.sleep(10)
                    continue
                ...
                response.raise_for_status()
                complexity = (
                    response.json().get("content", {}).get("complexity", "red").lower()
                )
                logger.debug(
                    "ThermoFisher classification score returned: '%s'", complexity
                )
                return complexity != "red", None
            except Exception:
                logger.exception(
                    "Fatal processing breakdown executing ThermoFisher API requests."
                )
                raise
