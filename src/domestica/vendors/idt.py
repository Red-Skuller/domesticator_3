...
class IDTSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="DOMESTICA_IDT_", env_file=".env", extra="ignore"
    )
...
class IDTEvaluator(ComplexityEvaluator):
    ...
    def __init__(self, product: str):
        ...
        self.threshold = {"eblocks": 10.0, "gblocks": 10.0, "genes": 10.0}.get(
            product, 0.0
        )
    ...
    def _get_token(self, force_refresh: bool = False) -> str:
        ...
        try:
            ...
            logger.debug(
                "IDT OAuth2 security connection established successfully. Valid for %s seconds.",
                body.get("expires_in", 3600),
            )
            return self._token
        except Exception:
            logger.exception(
                "Critical exception thrown while requesting authentication token from IDT server."
            )
            raise
    ...
    def evaluate(self, sequence: str) -> Tuple[bool, Optional[float]]:
        ...
        for attempt in range(6):
            logger.debug(
                "Dispatching sequence request to IDT endpoint. Attempt: %d/6. Sequence length: %d bp",
                attempt + 1,
                len(sequence),
            )
            try:
                ...
                if response.status_code == 401:
                    logger.warning(
                        "IDT endpoint returned 401 Unauthorized status. Forcing token regeneration."
                    )
                ...
                if response.status_code in (429, 500, 502, 503, 504):
                    ...
                    logger.warning(
                        "IDT server limits (Status: %d). Delaying exponential backoff: %.2f seconds.",
                ...
                if score is not None:
                    ...
                    logger.debug(
                        "IDT returned complexity metric: %f (Threshold: <= %f)",
                        score_val,
                        self.threshold,
                    )
                ...
                logger.debug(
                    "Complexity metrics missing. Falling back to explicit acceptance status: %s",
                    is_acceptable,
                )
                return is_acceptable, None
            except Exception:
                logger.exception(
                    "Exception encountered on evaluation cycle step %d of 6.",
                    attempt + 1,
                )
                if attempt == 5:
                    raise
