...
def _init_worker(vendor_target: Optional[str], product: str, verbose: bool) -> None:
    ...
    logger.debug(
        "Worker process initialized. Vendor: %s, Product: %s, Verbose: %s",
        vendor_target,
        product,
        verbose,
    )
...
def _worker_task(
    record: SequenceRecord, template_path: Path, min_length: int
) -> ResultRow:
    global _worker_evaluator
    ...
    try:
        ...
        logger.info(
            "Successfully optimized record ID: %s. Accepted: %s, Score: %s",
            record.record_id,
            accepted,
            score,
        )
        return ResultRow(
            ...
            protein_sequence=record.protein_sequence,
            naive_dna_sequence=naive,
            optimized_sequence=opt_seq,
            vendor_score=score,
            accepted=accepted,
            status="SUCCESS",
            optimized_record=opt_rec,
        )
    except Exception as e:
        logger.exception(
            "Execution failed for record ID: %s due to an unhandled exception.",
            record.record_id,
        )
        return ResultRow(
            record_id=record.record_id,
...
def optimize(
    ...
    output_path: Path = typer.Option(
        ..., "--output", "-o", help="Path for output file."
    ),
    template_path: Path = typer.Option(
        ..., "--template", "-t", exists=True, help="Path to template GenBank file."
    ),
    ...
    product: str = typer.Option(
        "eblocks", "--product", "-p", help="Product type for vendor evaluation."
    ),
    ...
    min_length: int = typer.Option(
        300, "--min-length", "-m", help="Minimum sequence length in base pairs."
    ),
    ...
    if max_workers < 1 or max_workers > 64:
        logger.error(
            "Invalid max_workers value: %d. Must be between 1 and 64.", max_workers
        )
    ...
    if input_path is not None:
        ...
        logger.info(
            "No input path provided. Transitioning optimization execution target directly to base template."
        )
        records = [
            SequenceRecord(record_id="optimized_template", protein_sequence=None)
        ]
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        future_to_record = {
            executor.submit(_worker_task, r, template_path, config.min_length): r
            for r in records
        }
        for future in concurrent.futures.as_completed(future_to_record):
            ...
            try:
                ...
                results.append(res)
                logger.debug(
                    "Retrieved future result execution status for record %s: %s",
                    rec.record_id,
                    res.status,
                )
            except Exception:
                logger.exception(
                    "Critical error retrieving process future result for record %s.",
                    rec.record_id,
                )
